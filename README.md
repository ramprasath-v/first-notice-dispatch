...
### Evidence remediation and voice-assisted intake

A submitted damage photo may contain useful damage evidence while leaving vehicle identity or incident details unresolved. FirstNotice pauses the same durable claim and asks only for the evidence that is blocking progress.

The claimant can upload a clearer vehicle image to resolve vehicle identity. When incident details remain missing, FirstNotice can request a voice response and use Gemini to extract grounded incident date, time, factual accident context, and injury information.

If the new evidence is sufficient and non-consequential, the workflow continues automatically. If the claimant reports a potentially consequential condition such as injury, FirstNotice pauses automation and routes the completed evidence package to an adjuster.

### Human review and inspection authorization

The adjuster receives a secure review link containing the current evidence package, claimant voice context, and the reason human judgment is required. Approve Inspection resumes the same durable workflow rather than starting a new process.

FirstNotice then creates the inspection Calendar event and sends the final Gmail handoff. Request More Info instead converts the adjuster's constrained instruction into one claimant action and resumes when that correction arrives.
...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gemini 3.5 Flash on Vertex AI   | Multimodal intake, evidence review, narrow new-document quality checks, and structured adjuster-summary drafting                 | Understands images, PDFs and claimant audio while returning Pydantic-validated structured output; Vertex AI uses Google Cloud identity and billing |
