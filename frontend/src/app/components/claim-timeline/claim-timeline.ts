...
export class ClaimTimeline {
  ...
  get activity(): AgentActivityItem[] {
    return normalizedEvents(this.events).flatMap((event) => activitiesFor(event).map((item, index) => ({
...
export function activityFor(event: ClaimEvent): Omit<AgentActivityItem, 'key' | 'timestamp'> | null {
...
export function normalizedEvents(events: ClaimEvent[]): ClaimEvent[] {
  ...
  return events
    ...
    .sort((left, right) => {
      ...
      return delta || eventKey(left.event).localeCompare(eventKey(right.event)) || left.index - right.index;
    })
    .flatMap(({ event }) => {
...
function activitiesFor(event: ClaimEvent): Array<Omit<AgentActivityItem, 'key' | 'timestamp'>> {
  switch (event.action) {
    case 'claim_intake_completed': return [{ agent: 'INTAKE AGENT', description: 'Analyzed the submitted multimodal evidence' }];
    case 'claim_review_completed': {
      if (event.to_status === 'awaiting_documents') return [
        { agent: 'REVIEW AGENT', description: 'Detected missing evidence required to continue' },
        { agent: 'WORKFLOW', description: 'Paused the claim while waiting for claimant evidence' },
      ];
    ...
    case 'document_received': return [{ agent: 'EVIDENCE', description: 'Received additional evidence from the claimant' }];
    case 'document_quality_checked': return [{ agent: 'REVIEW AGENT', description: 'Checked the new evidence quality' }];
    case 'claim_review_resumed': return [{ agent: 'WORKFLOW', description: 'Automatically resumed the existing claim' }];
    case 'missing_requirement_satisfied': return [{ agent: 'REVIEW AGENT', description: 'Accepted the requested evidence' }];
    case 'missing_requirement_still_unresolved': return [{ agent: 'REVIEW AGENT', description: 'Determined that replacement evidence is still needed' }];
    case 'human_review_requested': return [{ agent: 'WORKFLOW', description: 'Paused the claim for human review' }];
    case 'human_review_email_sent': return [{ agent: 'ADJUSTER REVIEW', description: 'Sent a secure review request through Gmail' }];
    case 'human_review_approved': return [{ agent: 'ADJUSTER REVIEW', description: 'Human approval received' }];
    case 'human_review_correction_requested': return [{ agent: 'ADJUSTER REVIEW', description: 'Requested a claimant correction' }];
    case 'human_review_resumed': return [{ agent: 'WORKFLOW', description: 'Automatically resumed the claim after human review' }];
    case 'google_calendar_event_created': return [{ agent: 'DISPATCHER', description: 'Scheduled the inspection in Google Calendar' }];
    case 'adjuster_packet_created': return [{ agent: 'ADJUSTER DISPATCH', description: 'Prepared the adjuster-ready claim packet' }];
    case 'adjuster_email_sent': return [{ agent: 'ADJUSTER DISPATCH', description: 'Sent the final claim handoff through Gmail' }];
    default: return [];
  }
}
...
function eventKey(event: ClaimEvent): string {
  const stableDetail = ['event_id', 'review_id', 'document_id', 'appointment_id', 'notification_id', 'idempotency_key']
